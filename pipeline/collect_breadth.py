...
def extract_asof(page_text: str) -> str:
    ...
    # Page is current product data; use latest Korean business-ish date only as last resort.
...
def download_plus_basket(n: str, title: str, suffix: str, expected: int) -> tuple[list[str], dict]:
    ...
    page = session.get(product_url, timeout=30); page.raise_for_status()
    asof = extract_asof(page.text)
    d = asof.replace('-', '')
    excel_url = f"{PLUS_BASE}/excel/product/pdf?n={n}&d={d}&title={quote(title)}"
    r = session.get(excel_url, timeout=60); r.raise_for_status()
    ...
    # The ETF's own six-digit ticker can appear in workbook metadata. Remove it if present.
    etf_ticker = {"006184": "152100", "006318": "301400"}.get(n)
    if etf_ticker in codes:
        codes.remove(etf_ticker)
    if len(codes) != expected:
        # Fallback: inspect embedded list, useful for diagnostics and product-page changes.
        ...
        if m:
            try:
                ...
                pass
        raise RuntimeError(f"PLUS Excel stock count mismatch {title}: expected={expected}, got={len(codes)}, embedded={len(embedded)}, asof={asof}, codes_head={codes[:12]}")
    symbols = [f"{c}.{suffix}" for c in codes]
    return symbols, {"product_url": product_url, "excel_url": excel_url, "asof": asof, "stock_rows": len(codes), "payload_bytes": len(r.content)}


def fallback_universe() -> tuple[list[str], dict]:
    k200 = requests.get(K200_FALLBACK, timeout=30).json(); k150 = requests.get(K150_FALLBACK, timeout=30).json()
    ...
    return symbols, {"source": "public_github_constituent_cache_fallback", "kospi200_count": len(a), "kosdaq150_count": len(b), "combined_count": len(symbols), "urls": [K200_FALLBACK, K150_FALLBACK]}
...
def fetch_universe() -> tuple[list[str], dict]:
    try:
        ...
        meta = {"source": "PLUS_official_excel_pdf_baskets", "kospi200_count": 200, "kosdaq150_count": 150, "combined_count": 350, "kospi200": m200, "kosdaq150": m150}
    except Exception as exc:
        symbols, meta = fallback_universe(); meta["official_plus_error"] = str(exc)
...
def collect_prices(symbols: list[str], start: str) -> pd.DataFrame:
    ...
    if px.empty: return pd.DataFrame(columns=["date", "ticker", "close"])
    if isinstance(px.columns, pd.MultiIndex):
        if "Close" not in px.columns.get_level_values(0): return pd.DataFrame(columns=["date", "ticker", "close"])
        close = px["Close"].copy()
    else:
...
def compute_breadth(prices: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker, g in prices.groupby("ticker"):
        g = g.sort_values("date").copy(); g["ma20"] = g["close"].rolling(20, min_periods=20).mean(); g["sd20"] = g["close"].rolling(20, min_periods=20).std(ddof=0)
        g["upper2"] = g["ma20"] + 2*g["sd20"]; g["eligible"] = g["upper2"].notna(); g["breakout"] = g["eligible"] & (g["close"] > g["upper2"])
        parts.append(g[["date", "ticker", "eligible", "breakout"]])
    z = pd.concat(parts, ignore_index=True); eligible = z[z["eligible"]].copy()
    agg = eligible.groupby("date").agg(universe_count=("ticker", "nunique"), breakout_count=("breakout", "sum")).reset_index()
    agg["breadth_pct"] = agg["breakout_count"]/agg["universe_count"]; agg["breadth_5dma"] = agg["breadth_pct"].rolling(5, min_periods=1).mean(); agg["slope"] = agg["breadth_5dma"].diff(); agg["acceleration"] = agg["slope"].diff(); agg["date"] = agg["date"].dt.strftime("%Y-%m-%d")
    return agg


def upsert(path: Path, new: pd.DataFrame, keys: list[str]) -> None:
    old = pd.read_csv(path) if path.exists() and path.stat().st_size else pd.DataFrame(); out = pd.concat([old,new],ignore_index=True) if not old.empty else new.copy(); out = out.drop_duplicates(keys,keep="last").sort_values(keys); out.to_csv(path,index=False)


def main() -> None:
    symbols, meta = fetch_universe(); start = (datetime.now(KST).date()-timedelta(days=430)).isoformat(); prices = collect_prices(symbols,start); breadth = compute_breadth(prices)
    if breadth.empty: raise RuntimeError("Breadth calculation produced no rows")
    upsert(DERIVED/"breadth_bb20_2sigma_daily.csv",breadth,["date"]); latest=breadth.iloc[-1].to_dict()
    status={"generated_at_kst":datetime.now(KST).isoformat(),"start_requested":start,"symbol_count":len(symbols),"price_rows":int(len(prices)),"breadth_rows":int(len(breadth)),"latest":latest,**meta}
